...
const LandingStyle = () => (
  <style>{`
    ...
    .lp-root { font-family: 'Inter', ui-sans-serif, system-ui, sans-serif; background: #050507; color: #F4F5F7; }
    ...
    .lp-card { background: #0B0C10; border: 1px solid rgba(255,255,255,0.08); }
...
const TickerTape = () => (
  <div className="border-y border-white/10 bg-white/[0.02] overflow-hidden py-2.5">
...
const Accordion = ({ q, a, open, onToggle }) => (
  <div className="border-b border-white/10">
    <button onClick={onToggle} className="w-full flex items-center justify-between gap-4 py-5 text-left">
      ...
      <ChevronDown size={18} className={`shrink-0 text-white/40 transition-transform duration-300 ${open ? "rotate-180" : ""}`} />
    </button>
    <div className={`grid transition-all duration-300 ease-out ${open ? "grid-rows-[1fr] opacity-100 pb-5" : "grid-rows-[0fr] opacity-0"}`}>
      <div className="overflow-hidden">
        <p className="text-sm text-white/60 leading-relaxed max-w-2xl">{a}</p>
      </div>
    </div>
  </div>
);
...
const Hero = ({ onGetStarted, onSignIn, stats }) => (
  <section className="relative pt-16 pb-20 md:pt-24 md:pb-28 px-4 overflow-hidden">
    ...
    <div className="relative max-w-5xl mx-auto text-center">
      <div className="lp-rise-1 inline-flex items-center gap-2 bg-white/5 border border-white/10 rounded-full pl-2 pr-4 py-1.5 mb-8">
        <span className="w-2 h-2 rounded-full bg-emerald-400 lp-pulse-dot" />
        <span className="text-xs font-semibold text-white/70">Live rule tracking for prop firm evaluations</span>
      </div>

      <h1 className="lp-rise-2 lp-display text-5xl md:text-7xl font-bold leading-[1.05] mb-6 text-white">
        Trade with data.<br />Not vibes.
      </h1>
      <p className="lp-rise-2 text-lg md:text-xl text-white/60 max-w-2xl mx-auto mb-10 leading-relaxed">
      ...
      <div className="lp-rise-3 flex flex-col sm:flex-row items-center justify-center gap-3 mb-16">
        ...
        <button onClick={onSignIn} className="text-white/70 hover:text-white font-semibold px-7 py-3.5 transition-colors">
          Sign in
        </button>
      </div>
      ...
      <div className="lp-rise-4 relative max-w-3xl mx-auto">
        <div className="lp-card rounded-2xl p-5 md:p-8">
          ...
          <div className="flex items-center justify-between mt-2 pt-4 border-t border-white/10">
            <div>
              <div className="text-[11px] uppercase tracking-wide text-white/40 font-semibold">Net P&L, this month</div>
              <div className="lp-mono text-2xl font-bold text-emerald-400">+$4,218.60</div>
            </div>
            <div className="text-right">
              <div className="text-[11px] uppercase tracking-wide text-white/40 font-semibold">Daily loss limit</div>
              <div className="lp-mono text-2xl font-bold text-white">12% used</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {(stats?.traders || stats?.trades) && (
        <div className="lp-rise-4 flex flex-wrap items-center justify-center gap-x-10 gap-y-3 mt-14 text-white/50">
          {stats.traders != null && <span className="text-sm"><span className="lp-mono font-bold text-white">{stats.traders.toLocaleString()}</span> traders</span>}
          {stats.trades != null && <span className="text-sm"><span className="lp-mono font-bold text-white">{stats.trades.toLocaleString()}</span> trades logged</span>}
          {stats.posts != null && <span className="text-sm"><span className="lp-mono font-bold text-white">{stats.posts.toLocaleString()}</span> community posts</span>}
        </div>
      )}
    </div>
  </section>
...
const ProblemSection = () => (
  <section className="py-20 md:py-28 px-4 border-t border-white/10">
    <div className="max-w-5xl mx-auto">
      <div className="max-w-2xl mb-14">
        <h2 className="lp-display text-3xl md:text-4xl font-bold text-white mb-4">Most traders don't fail on strategy. They fail on discipline.</h2>
        <p className="text-white/50 text-lg">A losing streak rarely comes from one bad idea — it comes from three or four small breaks in a row that nobody was tracking in real time.</p>
      </div>
...
const FLAGSHIP = [
  ...
  },
  {
    ...
    stat: { label: "Max drawdown", value: "3.2 / 10%", color: "text-white" },
  },
];

const FlagshipSection = () => (
  <section id="features" className="py-20 md:py-28 px-4 border-t border-white/10">
    <div className="max-w-5xl mx-auto">
      <div className="max-w-2xl mb-16">
        <h2 className="lp-display text-3xl md:text-4xl font-bold text-white mb-4">Built around the moments that actually break traders.</h2>
      </div>
        ...
        {FLAGSHIP.map((f, i) => (
          <div key={i} className={`grid md:grid-cols-2 gap-10 items-center ${i % 2 === 1 ? "md:[&>*:first-child]:order-2" : ""}`}>
            <div>
              ...
              <h3 className="lp-display text-2xl md:text-3xl font-bold text-white mt-2 mb-4">{f.title}</h3>
            ...
            <div className="lp-card rounded-2xl p-8 flex flex-col items-center justify-center gap-4 aspect-[4/3]">
              <f.icon size={40} className="text-white/20" />
              <div className="text-center">
                <div className="text-[11px] uppercase tracking-wide text-white/40 font-semibold">{f.stat.label}</div>
...
const MoreFeaturesSection = () => (
  <section className="py-20 md:py-28 px-4 border-t border-white/10">
    <div className="max-w-5xl mx-auto">
      <h2 className="lp-display text-2xl md:text-3xl font-bold text-white mb-12">Everything else you'd expect, and then some.</h2>
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {MORE_FEATURES.map((f, i) => (
          <div key={i} className="lp-card lp-card-hover rounded-xl p-5">
            <f.icon size={18} className="text-white/40 mb-3" />
...
const FAQSection = () => {
  ...
  return (
    <section id="faq" className="py-20 md:py-28 px-4 border-t border-white/10">
      <div className="max-w-3xl mx-auto">
        <h2 className="lp-display text-3xl md:text-4xl font-bold text-white mb-12">Questions, answered.</h2>
        <div>
          {FAQS.map((f, i) => (
...
const FinalCTA = ({ onGetStarted }) => (
  <section className="relative py-24 px-4 border-t border-white/10 overflow-hidden">
    ...
    <div className="relative max-w-2xl mx-auto text-center">
      <h2 className="lp-display text-3xl md:text-5xl font-bold text-white mb-5">Start trading like the data's actually watching.</h2>
      <p className="text-white/55 mb-9 text-lg">Free to start. No credit card. Your first trade takes under a minute to log.</p>
...
const Nav = ({ onGetStarted, onSignIn }) => {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-white/10 bg-[#050507]/85 backdrop-blur-xl">
      <div className="max-w-6xl mx-auto px-4 flex items-center justify-between h-16">
        <LogoFull size={28} forceLight />
        <nav className="hidden md:flex items-center gap-8">
          {NAV_LINKS.map((l) => <a key={l.label} href={l.href} className="text-sm font-medium text-white/60 hover:text-white transition-colors">{l.label}</a>)}
        </nav>
        <div className="hidden md:flex items-center gap-3">
          <ThemeToggle />
          <button onClick={onSignIn} className="text-sm font-semibold text-white/70 hover:text-white transition-colors px-3 py-2">Sign in</button>
        ...
        <button onClick={() => setMobileOpen((o) => !o)} className="md:hidden text-white p-2">{mobileOpen ? <X size={22} /> : <Menu size={22} />}</button>
      </div>
      {mobileOpen && (
        <div className="md:hidden border-t border-white/10 px-4 py-4 space-y-3">
          {NAV_LINKS.map((l) => <a key={l.label} href={l.href} className="block text-sm font-medium text-white/70 py-1.5">{l.label}</a>)}
          <div className="flex gap-2 pt-2">
            <button onClick={onSignIn} className="flex-1 text-sm font-semibold text-white/80 border border-white/15 rounded-lg py-2.5">Sign in</button>
  ...
  <footer className="border-t border-white/10 py-14 px-4">
    <div className="max-w-6xl mx-auto grid sm:grid-cols-2 md:grid-cols-4 gap-10">
      <div>
        ...
        <p className="text-sm text-white/40 mt-4 max-w-[220px]">A trading journal and prop firm challenge tracker built for traders who want the data, not the vibes.</p>
        <div className="flex items-center gap-3 mt-5">
          <a href="https://x.com" className="w-8 h-8 rounded-lg bg-white/5 hover:bg-white/10 flex items-center justify-center text-white/50 hover:text-white transition-colors"><XLogoIcon size={14} /></a>
        </div>
      </div>
      <div>
        <h4 className="text-xs font-bold uppercase tracking-wide text-white/40 mb-4">Product</h4>
        <ul className="space-y-2.5">
          {NAV_LINKS.map((l) => <li key={l.label}><a href={l.href} className="text-sm text-white/60 hover:text-white transition-colors">{l.label}</a></li>)}
        </ul>
      </div>
      <div>
        <h4 className="text-xs font-bold uppercase tracking-wide text-white/40 mb-4">Resources</h4>
        <ul className="space-y-2.5">
          {RESOURCES_LINKS.map((l) => <li key={l.label}><a href={l.href} className="text-sm text-white/60 hover:text-white transition-colors">{l.label}</a></li>)}
        </ul>
      </div>
      <div>
        <h4 className="text-xs font-bold uppercase tracking-wide text-white/40 mb-4">Legal</h4>
        <ul className="space-y-2.5">
    ...
    <div className="max-w-6xl mx-auto mt-12 pt-8 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-white/30">
