...
const RiskOfRuinCalculator = () => {
  ...
  const result = useMemo(() => {
    ...
    // Expressed in "units of risk": how many consecutive-equivalent losing
    // units would wipe out the ruin threshold.
    const unitsToRuin = ruinFrac / riskFrac;

    // Edge per trade in R: expected value in R-multiples.
    const edgeR = p * b - q;

    let riskOfRuin;
    if (edgeR <= 0) {
      // No positive edge: ruin is effectively certain given enough trades.
      riskOfRuin = 1;
    } else {
      // Classic ruin formula adapted for R-multiples: odds ratio (q/(p*b))
      // raised to the number of risk-units needed to reach the threshold.
...
// ---------------------------------------------------------------------
// R-Multiple: express a trade's target/stop as a multiple of risk, or
// work backward from a desired R to the exact target price.
// ---------------------------------------------------------------------
const RMultipleCalculator = () => {
  ...
  const result = useMemo(() => {
    ...
    if (!entry || !stop || riskDist === 0) return null;
    const rewardDist = target ? Math.abs(t - e) : null;
    const rMultiple = rewardDist != null ? rewardDist / riskDist : null;
    return { riskDist, rewardDist, rMultiple };
  }, [entry, stop, target]);
    ...
    <div>
      <div className="grid sm:grid-cols-2 gap-x-4">
        ...
        <Field label="Stop Loss Price">
          <input type="number" step="any" className={inputCls} placeholder="e.g. 1.08220" value={stop} onChange={(e) => setStop(e.target.value)} />
        </Field>
        <Field label="Target Price (optional)">
          <input type="number" step="any" className={inputCls} placeholder="e.g. 1.08820" value={target} onChange={(e) => setTarget(e.target.value)} />
        </Field>
      </div>
      ...
      {result ? (
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded-lg p-4">
            <div className="text-xs text-[var(--text-muted)] mb-1">Risk Distance (1R)</div>
            <div className="text-xl font-bold text-[var(--text-primary)] tabular-nums">{fmt(result.riskDist, 5)}</div>
          </div>
          <div className="bg-[var(--accent)]/10 border border-[var(--accent)]/30 rounded-lg p-4">
            <div className="text-xs text-[var(--accent)] mb-1">R-Multiple</div>
            <div className="text-xl font-bold text-[var(--accent)] tabular-nums">
              {result.rMultiple != null ? `${fmt(result.rMultiple)}R` : "— set a target"}
            </div>
          </div>
        </div>
      ) : (
        <div className="text-xs text-[var(--text-faint)] bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded-lg p-4">Enter an entry and stop price to see the trade's R-multiple.</div>
      )}
    </div>
  );
};
...
export default function RiskToolsPage() {
  const [mode, setMode] = useState("ruin");
    ...
    <div className="p-4 md:p-8 max-w-2xl mx-auto space-y-5">
      <Card className="p-5 md:p-6">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h2 className="font-bold text-[var(--text-primary)] text-sm">Risk Tools</h2>
        </div>
        <p className="text-xs text-[var(--text-muted)] mb-5 leading-relaxed">Sanity-check your edge before it costs you money — estimate risk of ruin for a strategy, or work out a trade's reward-to-risk in R.</p>

        <div className="flex rounded-lg overflow-hidden border border-white/10 mb-5">
          {TABS.map((t) => (
            <button key={t.id} onClick={() => setMode(t.id)}
              className={`flex-1 py-2 text-sm font-medium transition-colors ${mode === t.id ? "bg-[var(--accent)] text-[var(--text-inverse)]" : "bg-[var(--bg-primary)] text-[var(--text-muted)] hover:text-[var(--text-secondary)]"}`}>
              {t.label}
            </button>
          ))}
        </div>

        {mode === "ruin" ? <RiskOfRuinCalculator /> : <RMultipleCalculator />}
      ...
      <Card className="p-5 flex items-start gap-3">
        ...
        <p className="text-xs text-[var(--text-tertiary)] leading-relaxed">
          Risk of ruin tells you whether your edge and position sizing can survive a losing streak. R-multiples let you compare every trade on the same scale, regardless of instrument or stop size — a 2R win is a 2R win whether it's gold or a forex pair.
        </p>
      </Card>
    </div>
  );
